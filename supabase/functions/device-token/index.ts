...
const body = z.discriminatedUnion('action', [
  z.object({ action: z.literal('register'), token: z.string().min(20).max(512), platform: z.string().max(30).nullable(), deviceId: z.string().max(200).nullable() }).strict(),
...
Deno.serve((req) => run(req, false, async (ctx) => {
  ...
  const { data, error } = await ctx.admin.from('push_tokens').upsert({
    ...
    device_id: input.deviceId, is_active: true, updated_at: new Date().toISOString(),
